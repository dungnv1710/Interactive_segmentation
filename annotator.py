...
class Annotator(object):
    ...
    def onmouse(self, event, x,y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN or event == cv2.EVENT_RBUTTONDOWN:
            ...
            if event == cv2.EVENT_RBUTTONDOWN:
                button = 3
            xr, yr = int(x + 0.5), int(y + 0.5)
            self.clicks = np.append(self.clicks,np.array([[xr,yr,(3-button)/2]],dtype=np.int64),axis=0)
            self.__predict()
    ...
    def main(self):
        ...
        while k!= 113:
            ...
            cv2.setMouseCallback('Annotator', self.onmouse)
            cv2.imwrite('test_out.jpg', self.merge[..., ::-1])
            cv2.imshow('Annotator', self.merge[..., ::-1])
